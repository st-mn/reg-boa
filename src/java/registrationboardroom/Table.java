/*
Defines several classes related to generating and managing a table for displaying reservations of a boardroom. 
...
Contains methods like computeReservations, generateUserReservations, generateAdminReservations, 
and computeTable to handle table generation and data retrieval.
generateTable is used to generate the actual HTML table based on the collected data.

Overall, the Table class is responsible for creating a dynamic table to display reservations for a boardroom. 
It uses other helper classes (Event, Day, and DateComparator) to store and organize data. 
The class allows generating tables for both user reservations and admin reservations, 
and it handles different time intervals for the table (e.g., by hours, 30 minutes, or 15 minutes). 
